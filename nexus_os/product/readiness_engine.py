...
def score_readiness_field(runtime: dict[str, Any]) -> list[ReadinessPane]:
    ...
    now_score = 3.0 + (1.0 if history else 0.0) + _mode_boost(ui, "now")
    panes.append(
        ReadinessPane(
            id="pane:now",
            title="Now",
            ...
            actions=["continue", "focus", "summarize"],
            reason="top current objective and best next move anchor the workspace",
        )
    )

    if recent:
        changed_score = 1.5 + 0.4 * len(recent) + 0.3 * len(active_runs) + _mode_boost(ui, "what_changed")
        panes.append(
            ReadinessPane(
                ...
                score=changed_score,
                summary=" | ".join(recent),
                actions=["review", "compare", "pick up"],
    ...
    if active_runs:
        ...
        motion_score = 2.0 + 0.8 * len(active_runs) + _mode_boost(ui, "in_motion")
        panes.append(
            ReadinessPane(
                id="pane:in_motion",
                title="In Motion",
                ...
                score=motion_score,
                summary=run_states,
                actions=["inspect", "pause", "resume", "retry"],
                reason="ongoing jobs and retries deserve live workspace weight",
            )
        )

    if pending_approvals:
        approval_score = 2.8 + 0.9 * len(pending_approvals) + _mode_boost(ui, "needs_you")
        ...
        panes.append(
            ReadinessPane(
                ...
                actions=["approve", "inspect", "revise"],
    ...
    prepared_score = 1.2 + 0.5 * len(messages) + _mode_boost(ui, "prepared")
    panes.append(
        ReadinessPane(
            id="pane:prepared",
            ...
            actions=list(ui.get("quick_actions", ["continue", "branch", "launch"]))[:4],
    ...
    if recent_memories:
        ...
        context_score = 1.0 + 0.5 * len(recent_memories) + _mode_boost(ui, "context")
        panes.append(
            ReadinessPane(
                id="pane:context",
                title="Context",
