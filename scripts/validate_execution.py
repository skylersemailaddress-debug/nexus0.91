...
def _validate_scenario_specific(data: dict[str, Any], expected_scenario: str) -> list[str]:
    ...
    if expected_scenario == "resume_run" and run.get("status") not in {"resumed", "active"}:
    ...
    if expected_scenario == "fail_repair_revalidate":
        ...
        elif not any(int(job.get("retries", 0) or 0) >= 1 for job in revalidated):
...
def _validate_file(path: Path, expected_name: str, ref_time: datetime, max_age_seconds: int) -> dict[str, Any]:
    ...
    scenario = expected_name.replace(".json", "")
    details = _validate_common_payload(data, scenario)
    ...
    details.extend(_validate_scenario_specific(data, scenario))
...
def main() -> int:
    ...
    checks = [_validate_file(evidence_dir / name, name, ref_time, max_age_seconds) for name in REQUIRED]
